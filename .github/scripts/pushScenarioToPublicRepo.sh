...
HELM_SCENARIO_VERSION="$4"
SCENARIO_APP_VERSION="$5"
...
sed -i 's/\(.*version:.*\)/version: '"$HELM_SCENARIO_VERSION"'/g' Charts/qtest-chart/Chart.yaml
sed -i 's/\(.*appVersion:.*\)/appVersion: '"$SCENARIO_APP_VERSION"'/g' Charts/qtest-chart/Chart.yaml
